                ...
                try:
                    ...
                    if isinstance(num_e, sp.sin) and cero_en(num_e.args[0], h_val):
                        ...
                        if sp.simplify(dn - (1 - sp.cos(den_e))) == 0:
                            ...
                            fs_manual = sp.cancel(sp.expand_trig(f_expr))
                            if fs_manual != f_expr:
                                pasos.append(f"  Aplicando identidad trig: f(x) = {fs_manual}")
                                try: pasos.append(f"  f({h_str}) = {sp.simplify(fs_manual.subs(x, h_val))}")
                                except Exception: pass
                            else:
                                ff = sp.cancel(sp.factor(num_e) / sp.factor(den_e))
                                if ff != f_expr:
                                    pasos.append(f"  Factorizando y cancelando: f(x) = {ff}")
                                    try: pasos.append(f"  f({h_str}) = {sp.simplify(ff.subs(x, h_val))}")
                                    except Exception: pass
                                else:
...
class CalculadoraTrigExtendida(CalculadoraLimitesApp):
    ...
    def _resolver_trig_notable(self, expr, x, p):
        ...
        # Intentar simplificación trigonométrica manual (identidades)
        # sin²+cos²=1, tan=sin/cos, etc.
        fs_manual = sp.cancel(sp.expand_trig(expr))
        if fs_manual != expr:
            try:
                v = sp.simplify(fs_manual.subs(x, p))
                nv = complex(v).real
                if not math.isnan(nv) and not math.isinf(nv):
                    return v, f"Identidad trigonométrica aplicada: f(x) = {fs_manual}"
            except Exception: pass

        # Intentar factorizar numerador y denominador por separado y cancelar
        try:
            num_f = sp.factor(num)
            den_f = sp.factor(den)
            cancelado = sp.cancel(num_f / den_f)
            if cancelado != expr:
                v = sp.simplify(cancelado.subs(x, p))
                nv = complex(v).real
                if not math.isnan(nv) and not math.isinf(nv):
                    return v, f"Factorización y cancelación: f(x) = {cancelado}"
        except Exception: pass
