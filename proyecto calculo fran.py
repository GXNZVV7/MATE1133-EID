...
class CalculadoraLimitesApp(ctk.CTk):
    ...
    def _definir_limite_manual(self):
        ...
        # Determinar color del punto según proximidad al límite real
        if self._limite_correcto is not None:
            diferencia = abs(valor_ingresado - self._limite_correcto)
            if diferencia < 0.01:
                color_punto = '#A5D6A7'   # verde  — correcto
            elif diferencia < 0.5:
                color_punto = '#FFD54F'   # amarillo — aproximado
            else:
                color_punto = '#FF6B6B'   # rojo — incorrecto
        else:
            color_punto = '#A5D6A7'       # verde por defecto si no hay referencia

        # Dibujar el punto que el usuario definió (color según resultado)
        self.ax.plot(
        ...
        if self._limite_correcto is not None:
            if diferencia < 0.01:
                self.label_verificacion.configure(
                    ...
                    text_color="#A5D6A7"   # verde
                )
            elif diferencia < 0.5:
                self.label_verificacion.configure(
                    text=f"⚠ Casi. El límite real es {self._limite_correcto:.4f}, "
                         f"tu valor se aleja en {diferencia:.4f}.",
                    text_color="#FFD54F"   # amarillo
                )
            else:
                self.label_verificacion.configure(
                    text=f"✗ Incorrecto. El límite real es {self._limite_correcto}, "
                         f"no {valor_ingresado}.",
                    text_color="#FF6B6B"   # rojo
                )
