...
def _lateral(expr, p, dir):
    try:
        r = sp.limit(expr, x, p, dir)
        return r if (not isinstance(r, AccumulationBounds)) else sp.nan
    except: return sp.nan
...
def calcular_limite(f_expr, h_val):
    ...
    if h_val in (sp.oo, -sp.oo):
        ...
        try:
            r = sp.limit(f_expr, x, h_val)
            if not isinstance(r, AccumulationBounds): return r
        except: pass
    ...
    # [1] Analítico primario: límites laterales con sp.limit()
...
def generar_pasos(f_expr, h_val, h_str, res):
    ...
    if h_val in (sp.oo, -sp.oo):
        ...
        P.append(f"Tipo: Límite al {sig}∞  [analítico primario: sp.limit()]")
        if f_expr.is_rational_function(x):
            try:
                num, den = sp.fraction(sp.cancel(f_expr))
                gn, gd = sp.degree(sp.Poly(num,x)), sp.degree(sp.Poly(den,x))
        ...
        # Paso 1 – sp.limit() bilateral
        P.append("Paso 1 [ANALÍTICO PRIMARIO]: sp.limit() bilateral")
        try:
            ld = sp.limit(f_expr, x, h_val, '+')
            li = sp.limit(f_expr, x, h_val, '-')
            def _fmt(v): return "no converge" if isinstance(v, AccumulationBounds) else str(v)
            P += [f"  lím⁺ = {_fmt(ld)}", f"  lím⁻ = {_fmt(li)}"]
            if ld == li and not isinstance(ld, AccumulationBounds):
                P.append("  Laterales coinciden ✔ → límite existe")
            else:
                P.append("  Laterales DISTINTOS o no convergentes ✗ → límite NO existe")
        except Exception as e:
            P.append(f"  sp.limit() no resolvió: {e}")

        # Paso 2 – sustitución directa
        P.append("Paso 2 [ANALÍTICO SECUNDARIO]: Sustitución directa")
        vd = _sust(f_expr, h_val)
        if vd is not None:
            P.append(f"  f({h_str}) = {vd}  ✔")
        else:
            P.append(f"  f({h_str}) → indeterminación")
            if f_expr.has(*TRIG):
                P.append("Paso 3: Límites notables trigonométricos")
                ...
                try:
                    ...
                    if no_existe:
                        P += ["  Laterales distintos / función oscilatoria.",
                              "  → El límite NO existe."]
                    else:
                        P.append("  [Respaldo numérico bilateral aplicado]")

        if no_existe:
            P += ["", "⚠ El límite NO existe:",
                  "  · Límites laterales distintos, o",
                  "  · Función oscilatoria sin convergencia."]
...
class App(ctk.CTk):
    ...
    def _dibujar(self, f_expr, h_val, res, no_existe):
        ...
        if y_ok:
            ym = sum(y_ok)/len(y_ok)
            mg = max((max(y_ok)-min(y_ok))*0.6, 5)
